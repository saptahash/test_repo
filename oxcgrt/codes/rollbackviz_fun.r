...
tilemap.regionwise <- function(plot_rollback, region_name){
  ...
  ggplot(plot_rollback %>% arrange(CountryCode, Date) %>% ungroup() %>%
         ...
         aes(y = forcats::fct_rev(CountryCode), x = Date, fill = openness_risk)) + #scales::rescale(openness_risk, to = c(0,1)))) + 
    geom_tile(width = 0.9, height = 0.9) +
    scale_fill_viridis_c(name = "Risk of Openness", na.value = "gray", begin = 0, end = 1, direction = -1,breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1) ) +
